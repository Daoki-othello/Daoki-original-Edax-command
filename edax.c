...
void help_book(void) 
{
	...
	printf("  w_deviate <n1> <n2>   add positions by deviating between a minimum score <n1> and\n  a maximum score <n2>.\n");
	...
	printf("  auto_learning       learn book automatically.\n");
}
...
void remove_first_line(const char *filename) {
    ...
    char temp_[1024][1024];
	int i = 0;

    // 一行目を読み飛ばす
    char buffer[1024];
    if (fgets(buffer, sizeof(buffer), file) == NULL) {
        // ファイルが空であった場合、エラーメッセージを表示
        fclose(file);
        return;
    }
...
void edit_first_line_edaxvsedax(const char *filename, const int times, const int randomness, const char *kifu__, char *message) {
    FILE *file = fopen(filename, "r");
    if (file == NULL) {
        perror("a");
        return;
	...
	for (;;) {
		...
		// proceed by reading a command
			...
			// opening book commands
				...
				// add position using the "deviate algorithm" with width
				} else if (strcmp(book_cmd, "w_deviate") == 0) {
					val_1 = 2; book_param = parse_int(book_param, &val_1); BOUND(val_1, -120, 120, "minimum score");
					val_2 = 4; book_param = parse_int(book_param, &val_2); BOUND(val_2, -120, 120, "maximum score");
					book_width_deviate(book, play->board, val_1, val_2);
				// auto learning
				}else if (strcmp(book_cmd, "auto_learning") == 0) {
					FILE *learning_list;
					FILE *learned_log;

					size_t num_lines_ing = 0;
					size_t num_lines_ed = 0;
					char learning_buffer[256];
					char learned_buffer[256];
					char message[256];

					char ing_name[] = "learning_list.txt";
					char ed_name[] ="learned_log.txt"; 

					int param1, param2;
					int randomness, times;
					int randomness_, times_;
					bool first;
					char kifu[200]="";
					char kifu_[200]="";
					char kifu_all[200]="";
					options.auto_store=FALSE;
					play_new(play);
					...
					while(true){
						...
						num_lines_ing = 0;
						num_lines_ed = 0;
						char **learning_lines = NULL;
						char **learned_lines = NULL;
						strcpy(learning_buffer,"");
						strcpy(learned_buffer,"");
						strcpy(kifu,"");
						strcpy(kifu_,"");

						learning_list = fopen(ing_name,"r");
						learned_log = fopen(ed_name,"r");
						book->search = play->search;
						book->search->options.verbosity = book->options.verbosity;
						book_param = parse_word(param, book_cmd, FILENAME_MAX);

						if(learning_list == NULL){
							printf("%s file is not founded",ing_name);
							fclose(learning_list);	
							fclose(learned_log);
							play_new(play);
							break;
						}
						if(learned_log == NULL){
							printf("%s file is not founded",ed_name);
							fclose(learning_list);	
							fclose(learned_log);
							play_new(play);
							break;
						...
						//leaning_listの内容を保存
						while (fgets(learning_buffer, sizeof(learning_buffer), learning_list) != NULL) {
							learning_lines = realloc(learning_lines, (num_lines_ing + 1) * sizeof(char *));
							learning_lines[num_lines_ing] = strdup(learning_buffer);
							num_lines_ing++;
						}
						//learned_listの内容を保存
						while (fgets(learned_buffer, sizeof(learned_buffer), learned_log) != NULL) {
							learned_lines = realloc(learned_lines, (num_lines_ed + 1) * sizeof(char *));
							learned_lines[num_lines_ed] = strdup(learned_buffer);
							num_lines_ed++;
						}
						...
						//deviate
						if (sscanf(learning_lines[0], "[%d,%d]%99s", &param1, &param2, kifu) == 3) {
							printf("\ndeviate: %s\nrelative_error = %d, absolute_error = %d\n\n",kifu,param1,param2);
							string_to_lowercase(kifu);
							play_game(play, kifu);
							play_print(play, stdout); putchar('\n');
							book_deviate(book,play->board,param1,param2);
							//fixing book
							book_fix(book); // do nothing (or edax is buggy)
							book_link(book); // links nodes
							book_negamax(book); // negamax nodes
							book_sort(book); // sort moves
							book_save(book,options.book_file);

							//learned_log.txtを編集
							learned_log = fopen(ed_name,"w");
							for (size_t i = 0; i < num_lines_ed; i++) {
								fputs(learned_lines[i], learned_log);
								free(learned_lines[i]);
							}
							learned_lines = realloc(learned_lines, (num_lines_ed + 1) * sizeof(char *));
							fputs("\n", learned_log);
							Intrim(learning_lines[0]);
							fputs(learning_lines[0], learned_log);
							free(learned_lines);
							fclose(learned_log);

							//learning_list.txtを編集
							remove_first_line(ing_name);

							book->options.verbosity = book->search->options.verbosity;
							book->search->options.verbosity = options.verbosity;
						}
						//width_deviate
						else if (sscanf(learning_lines[0], "w[%d,%d]%99s", &param1, &param2, kifu) == 3){
							printf("\nwidth_deviate: %s\nminimum = %d, maximum = %d\n\n",kifu,param1,param2);
							string_to_lowercase(kifu);
							play_game(play, kifu);
							play_print(play, stdout); putchar('\n');
							book_width_deviate(book,play->board,param1,param2);
							//fixing book
							book_fix(book); // do nothing (or edax is buggy)
							book_link(book); // links nodes
							book_negamax(book); // negamax nodes
						...
						//edax vs edax
						else if (sscanf(learning_lines[0], "%d-%d,%s", &times, &randomness, kifu) == 3){
							printf("\nedax vs edax: %s\ntimes = %d, randomness = %d\n\n",kifu,times,randomness);
							options.book_randomness = randomness;
							string_to_lowercase(kifu);
							play_game(play, kifu);
							strcpy(kifu_all,kifu);
							first = true;
							while(!play_is_game_over(play)){	
								play_go(play, true);
								if(first){
									for (int i = 0; i < 4; i++) {
										// カーソルを一行上に移動
								...
								if(strcmp(move_to_string(play_get_last_move(play)->x,0,kifu_),"PA")){
									strcpy(kifu_,move_to_string(play_get_last_move(play)->x,0,kifu_));
									string_to_lowercase(kifu_);
									strcat(kifu_all,kifu_);
								}
								play_print(play, stdout); putchar('\n');
								fflush(stdout);
							}
							...
							if(num_lines_ed==0){
								...
								strcpy(message,"");
								fputs("\n", learned_log);
								sprintf(message,"%d-%d,%s",1,randomness,kifu);
								fputs(message, learned_log);
								free(learned_lines);
							}
							//最後の行が同じ学習内容の場合
							else if(sscanf(learned_lines[num_lines_ed-1], "%d-%d,%s", &times_, &randomness_, kifu_) >= 2&&randomness==randomness_&&strcmp(kifu,kifu_)==0){
								//learned_log.txtを編集
								for (size_t i = 0; i < num_lines_ed-1; i++) {
									fputs(learned_lines[i], learned_log);
									free(learned_lines[i]);
								}
								learned_lines = realloc(learned_lines, (num_lines_ed + 1) * sizeof(char *));
								strcpy(message,"");
								sprintf(message,"%d-%d,%s",times_+1,randomness,kifu);
								fputs(message, learned_log);
								free(learned_lines);
							}
							else{
							...
							if(times<=1){
								...
								edit_first_line_edaxvsedax(ing_name,times-1,randomness,kifu,message);			
							}
						}
						//fix
						else if(strcmp(learning_lines[0],"fix")==0){
							book_fix(book); // do nothing (or edax is buggy)
							book_link(book); // links nodes
							book_negamax(book); // negamax nodes
							book_sort(book); // sort moves
						...
						//end
						else if(strcmp(learning_lines[0],"end")==0||learning_lines[0]==NULL||strcmp(learning_lines[0],"\n")==0){
							//learned_log.txtを編集
							learned_log = fopen(ed_name,"w");
							for (size_t i = 0; i < num_lines_ed; i++) {
								fputs(learned_lines[i], learned_log);
						...
						//comment
						else if (sscanf(learning_lines[0], "//%99s" ,kifu) == 1){
							//learned_log.txtを編集
							learned_log = fopen(ed_name,"w");
							for (size_t i = 0; i < num_lines_ed; i++) {
								fputs(learned_lines[i], learned_log);
							...
							fputs("\n", learned_log);
							Intrim(learning_lines[0]);
							fputs(strcat("Invalid: ",learning_lines[0]), learned_log);
							free(learned_lines);
							fclose(learned_log);
							...
							printf("The command %s is invalid",learning_buffer);
						}
						play_new(play);
						fflush(stdout);
					}
