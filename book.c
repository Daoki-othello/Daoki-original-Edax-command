...
/**
 * @brief Deviate a position with width.
 *
 * @param position Position to expand.
 * @param book Opening book.
 * @param lower Error lower bound.
 * @param upper Error upper bound.
 * @param flg Switch mode.
 */
static void position_width_deviate(Position *position, Book *book, const int lower, const int upper, const int flg)
{
	Link *l;
	Board target[1];
	Position *child;
	...
	if(flg==0){
		...
		if (!position->done && lower <= position->score.value && position->score.value <= upper && board_count_empties(position->board) >= book->options.n_empties && !board_is_game_over(position->board)) {
			...
			// deviate all children close to the best move
			foreach_link(l, position) {
				if (lower <= l->score && l->score <= upper) {
					board_next(position->board, l->move, target);
					child = book_probe(book, target);
					position_width_deviate(child, book,-upper, -lower, 0);
				}
			}

			// expand the best remaining move
			if (lower <= position->leaf.score && position->leaf.score <= upper) {
				position->todo = true; book->stats.n_todo++;
				if (book->stats.n_todo % 10 == 0) bprint("Book deviate %d todo\r", book->stats.n_todo);
			}
		}
		...
		if (!position->done &&position->score.value <= upper && board_count_empties(position->board) >= book->options.n_empties && !board_is_game_over(position->board)) {
			position->done = true;

			// deviate all children close to the best move
			foreach_link(l, position) {
				if (position->score.value - l->score ==0) {
					board_next(position->board, l->move, target);
					child = book_probe(book, target);
					position_width_deviate(child, book,-upper, -lower, 1);
				}
			}

			// expand the best remaining move
			if (position->score.value - position->leaf.score ==0) {
				position->todo = true; book->stats.n_todo++;
				if (book->stats.n_todo % 10 == 0) bprint("Book deviate %d todo\r", book->stats.n_todo);
			}
		}
		...
		if (!position->done && lower <= position->score.value && board_count_empties(position->board) >= book->options.n_empties && !board_is_game_over(position->board)) {
			position->done = true;
...
// static void position_neighbor_deviate(Position *position, Book *book, const root_score, const int score ,const int relative_error, const int absolute_error, const int flg)
// {
// 	Link *l;
// 	Board target[1];
// 	Position *child;
...
// 	if(flg<0){
// 		if (!position->done &&position->score.value == root_score && board_count_empties(position->board) >= book->options.n_empties && !board_is_game_over(position->board)) {
// 			position->done = true;
...
// 					board_next(position->board, l->move, target);
// 					child = book_probe(book, target);
// 					position_neighbor_deviate(child, book, -root_score, -score, relative_error, absolute_error, 1);
// 				}
// 			}
...
// 						board_next(position->board, l->move, target);
// 						child = book_probe(book, target);
// 						position_width_deviate(child, book, root_score ,relative_error, absolute_error, -1);
// 					}
// 					else{
// 						board_next(position->board, l->move, target);
// 						child = book_probe(book, target);
// 						position_deviate(child, book, relative_error, 0, score - absolute_error, score + absolute_error);
// 						position_deviate(child, book, 0, relative_error, score - absolute_error, score + absolute_error);						
// 					}
// 				}
// 			}

// 			// expand the best remaining move
// 			if (score <= position->leaf.score) {
// 				position->todo = true; book->stats.n_todo++;
// 				if (book->stats.n_todo % 10 == 0) bprint("Book deviate %d todo\r", book->stats.n_todo);
// 			}
// 		}
...
/**
 * @brief Deviate a book with width.
 *
 * @param book opening book.
 * @param board Position to start from.
 ...
 */
void book_width_deviate(Book *book, Board *board, const int min_score, const int max_score)
{
	if(min_score>max_score){
		bprint("You must set for min<=max");
		return 0;
	}
	Position *root = book_probe(book, board);
	if (root) {
		...
		do {
			score = root->score.value;
			if(score<min_score){flg=-1;}
			else if(score>max_score){flg=1;}
			else{flg=0;}

			bprint("Book width_deviate min:%d max:%d\n", min_score, max_score);
			book_clean(book);
			position_width_deviate(root, book, min_score, max_score,flg);
			bprint("Book deviate %d todo\n", book->stats.n_todo);
		...
		bprint("Book width_deviate min:%d max:%d...finished\n", min_score, max_score);
	}
}
 ...
 * @param max_score maximum score.
 */
void book_neighbor_deviate(Book *book, Board *board, const int dev_score, const int relative_error, const int absolute_error)
{
	Position *root = book_probe(book, board);
	if (root) {
		int score;
		...
		do {
			score = root->score.value;
			if(score==dev_score){
				book_deviate(book,board,relative_error,absolute_error);
				break;
			}
			else if (score<dev_score){flg=-1;}
			else{flg=1;}

			bprint("Book neighbor_deviate expand_score:%d relative_error:%d absolute_error:%d\n", dev_score, relative_error, absolute_error);
			book_clean(book);
			position_neighbor_deviate(root, book, score, dev_score, relative_error, absolute_error,flg);
			bprint("Book deviate %d todo\n", book->stats.n_todo);
		...
		bprint("Book neighbor_deviate expand_score:%d relative_error:%d absolute_error:%d...finished\n", dev_score, relative_error, absolute_error);
	}
}
